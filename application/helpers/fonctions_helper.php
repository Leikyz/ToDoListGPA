...
{
    $CI = & get_instance();
    $requete = $CI->db->query("SELECT * FROM liste_motif_absence WHERE liste_motif_absence_id='" . $id . "'");
    $data = $requete->row_array();
...
function GetAbsenceIntervenant($activite_id, $intervenant_id, $affichage_libelle)
{
    ...
    if ($affichage_libelle == 'oui' and !empty($data))
    {
...
{
    $CI = & get_instance();
    $query = $CI->db->query(" SELECT * FROM service 
...
function Affiche_Liste_Services($type_ets, $type_affichage, $services, $var = NULL, $archive = NULL)
{
    ...
    foreach ($data as $key => $ligne_services)
    {
        ...
        if ($type_affichage == 'checkbox')
        {
            ...
            {
                ?>
                <input name="services[]" type="checkbox" value="<?php echo $ligne_services['service_id']; ?>" <?php if (in_array($ligne_services['service_id'], $var))
                { ?>checked<?php } ?> /><?php echo $service_nom; ?><br />
            <?php } else
            { ?>
        ...
        {
            ?>
            <option value="<?php echo $ligne_services['service_id']; ?>" <?php if ($ligne_services['service_id'] == $var)
            { ?>selected<?php } ?>><?php echo $service_nom; ?></option>
            <?php
        }
    }
}
...
//Liste les contacts d'un usager
function Liste_Contact_Champs($id)
{ //'tab' : tableau d'array
    $tab_contacts = array();
    $CI = & get_instance();
    //récupère les id des contacts dans la ligne de l'usager
...
function Get_Next_EP($id, $col_date, $date)
{ //$col_date = pec_date_debut ou pec_date_demande
    global $pdo;
...
/*
 * Fonction pour récupérer les droits sur les questionnaires générique
 * @param int $questionnaireId id de la ligne de la table liste_generale contenant le champs liste_generale_info si abesnt analyse tous les questionnaires
 * @return array
 */

function getDroitsActionsCategorieQuestionnaire($questionnaireId = NULL)
{
    global $pdo;
//tout refuser    
    $droitQuestionnaire['creation'] = false;
    $droitQuestionnaire['remplissage'] = false;
    $droitQuestionnaire['consultation'] = false;
//si analyse droits pour 1 questionnaire    
    if ($questionnaireId !== NULL)
    {
        //récupération JSON droits
        $questionnaireData = Get_ID_Champs("liste_generale", "liste_generale_id=" . $questionnaireId, "liste_generale_info", "");
        $tabDroits[0] = $questionnaireData['liste_generale_info'];
    } else // si nalayse droits pour tous les questionnaires
    {
        $tabDroits = [];
        $CI = & get_instance();
        $query = $CI->db->query("SELECT liste_generale_info FROM liste_generale WHERE liste_generale_type='questionnaire'");
        foreach ($query->result_array() as $row)
        {
            $tabDroits[] = $row['liste_generale_info'];
        }
    }


//test des droits pour chaque categorie de questionnaire
    foreach ($tabDroits as $jsonDroits)
    {
        $droits = json_decode($jsonDroits);
//CREATION
        if (array_intersect(explode(",", $_SESSION['services']), $droits->creation->services))
        {
            $droitQuestionnaire['creation'] = true;
            $droitQuestionnaire['remplissage'] = true;
            $droitQuestionnaire['consultation'] = true;
            return $droitQuestionnaire;
        }
        if (in_array($_SESSION['MM_UserGroup'], $droits->creation->niveaux))
        {
            $droitQuestionnaire['creation'] = true;
            $droitQuestionnaire['remplissage'] = true;
            $droitQuestionnaire['consultation'] = true;
        ...
        if (in_array($_SESSION['personnel_profession'], $droits->creation->fonctions))
        {
            $droitQuestionnaire['creation'] = true;
            $droitQuestionnaire['remplissage'] = true;
            $droitQuestionnaire['consultation'] = true;
        ...
        if (in_array($_SESSION['MM_Username'], $droits->creation->users))
        {
            $droitQuestionnaire['creation'] = true;
            $droitQuestionnaire['remplissage'] = true;
            $droitQuestionnaire['consultation'] = true;
...
//REMPLISSAGE
        ...
        if (in_array($_SESSION['MM_UserGroup'], $droits->remplissage->niveaux))
        {
            $droitQuestionnaire['remplissage'] = true;
            $droitQuestionnaire['consultation'] = true;
            return $droitQuestionnaire;
        ...
        if (in_array($_SESSION['personnel_profession'], $droits->remplissage->fonctions))
        {
            $droitQuestionnaire['remplissage'] = true;
            $droitQuestionnaire['consultation'] = true;
            return $droitQuestionnaire;
        }
        if (in_array($_SESSION['MM_Username'], $droits->remplissage->users))
        {
            $droitQuestionnaire['remplissage'] = true;
            $droitQuestionnaire['consultation'] = true;
            return $droitQuestionnaire;
...
//CONSULTATION
        ...
        if (in_array($_SESSION['MM_UserGroup'], $droits->consultation->niveaux))
        {
            $droitQuestionnaire['consultation'] = true;
            return $droitQuestionnaire;
        }

        if (in_array($_SESSION['personnel_profession'], $droits->consultation->fonctions))
        {
            $droitQuestionnaire['consultation'] = true;
            return $droitQuestionnaire;
        }
        if (in_array($_SESSION['MM_Username'], $droits->consultation->users))
        {
            $droitQuestionnaire['consultation'] = true;
            return $droitQuestionnaire;
        }
...
function getSMSBlackList()
{
    //-------config-------

        $url = 'https://api.allmysms.com/http/9.0/getBlacklist'; 
    ...
    } elseif (CLIENT == "APED-ESPOIR")
    {
        $login = 'apedespoir';    // identifant allmysms spécifique
    }else
    {
...
function creationTableauRubriques($type_usager = 'enfant', $ids_services = null, $noms_rubriques = null, $noms_onglets = null, $retour = 'array')
{
    ...
    if ($type_usager === "adulte" or $type_usager === "enfant")
    {
        ...
        $SQL_services = "";
        if ($ids_services != null)
        { //si des id de services sont demandés
        ...
        { //sinon selection des services de l'utilisateur
        ...
        if ($noms_rubriques != null)
        { //si des id de rubriques sont demandés
            $SQL_rubriques = "";
...
function testutf8($texte)
{
    ...
    if (file_exists($_SERVER['DOCUMENT_ROOT'] . '/encode_utf8.txt'))
    {
        $nouveau_texte = $texte;
    } else
    {
...
/**
 * Permet d'appeler un service REST en Curl
 * @param str 	$method 	POST, PUT, GET etc
 * @param str 	$url 		adresse web
...
function consoleDebug($data,$description="")
{
    $html = "";
    $coll;
    $trace = debug_backtrace();
    ...
    $html = "<script>console.log('PHP $file $line $description : ${coll}');</script>";
